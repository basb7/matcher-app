...
export async function handleServerLogin(data: any) {
  ...
  try {
    const res = await fetch(process.env.NEXT_PUBLIC_API_URL + API_ENDPOINTS.LOGIN, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return {
        ...
        message: res.status === 401 ? "Credenciales invalidas" : errorData.message || errorData.detail || errorData.error || "Error desconocido"
      }
    }
